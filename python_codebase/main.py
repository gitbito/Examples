...
@app.post(
    ...
    response_model=user_pydanticOut,
)
async def user_registration(user: user_pydanticIn):
    ...
    if len(user_info["username"]) < 5:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Username must be longer than 5 characters",
        )
...
async def get_product_list(limit: int = Query(100, le=100), skip: int = Query(0, ge=0)):

    response = await product_pydantic.from_queryset(
        Product.filter(id__gt=skip, id__lte=skip + limit)
    )
    return response
